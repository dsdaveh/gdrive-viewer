...
parse_email <- function(email) { 
    # str_replace is for demo (causes a potential error ... similiar handles, different domains treated as same people)
    str_replace(email, "@.*", "") 
}
...
for (i in 1:nrow(shared)) { ## need to purrrfect this
    ...
    pals[i] <- purrr::map_chr(xx, function(.x) #.x$type) %>% paste(collapse = ',')
        ifelse(.x$type == 'user', .x$emailAddress, .x$type)) %>% setdiff(me) %>% paste(collapse = ',')
    all_names <- str_split(pals[i], ',') %>% unlist() %>% parse_email()
    people <- c(people, all_names) %>% unique()
}
...
for (i in 1:nrow(shared)) { ## need to purrrfect this
    ...
    owner <- shared[i, ]$drive_resource[[1]]$owners[[1]]$emailAddress %>% parse_email()
    for (j in 1:length(xx)) {
        if (xx[[j]]$role == 'owner') next
        share_with_df <- bind_rows(share_with_df,
                                   tibble(from = owner, to = xx[[j]]$emailAddress %>% parse_email()))
    } 
    
}
people2 <- c(share_with_df$to, share_with_df$from) %>% unique() 
swu <- unique(share_with_df)
vn <- visNetwork(data.frame(id = people2, title = people2), swu)
```
